...
def train_combined(data_version):
    ...
    print("Training on normalized bin ID")
    print(f"Version of the data: {data_version}")

    bin_sizes = ["100kb", "500kb", "1mb"]

    for bin_size in bin_sizes:
        print(f"Training on bin size {bin_size}")

        X_local_train, X_bin_id_1mb_train, y_train, X_local_val, X_bin_id_1mb_val, y_val, _, X_bin_id_1mb_test, _ = load_splits(data_version, requested_splits=["train", "val", "test"], bin_size=bin_size, requested_features=["bin_id"])

        all_bins = np.concatenate((X_bin_id_1mb_train, X_bin_id_1mb_val, X_bin_id_1mb_test), axis=0)
        unique_bins = set(all_bins)
        n_bins = len(unique_bins) # Divide individual IDs by this to normalize them
        ...
        print(f"X_bin_id_1mb_normalized_train shape: {X_bin_id_1mb_normalized_train.shape}")

        train_dataset = TensorDataset(X_local_train, X_bin_id_1mb_normalized_train, y_train)
        val_dataset = TensorDataset(X_local_val, X_bin_id_1mb_normalized_val, y_val)
        ...
        model = ModularModel(use_bin_id_norm=True)

        print("First two rows of normalized bin IDs:", X_bin_id_1mb_normalized_train[:2])
        with torch.no_grad():
            model(local_context=X_local_train[:2], bin_id_norm=X_bin_id_1mb_normalized_train[:2])
        ...
        for epoch in range(epochs):
            ...
            for xb_local, xb_region, yb in train_loader:
                pred = model(local_context=xb_local, bin_id_norm=xb_region)
        ...
        model_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/models/combined/{data_version}/binnorm"
        os.makedirs(model_dir, exist_ok=True)
        torch.save(model.state_dict(), f"{model_dir}/model_{bin_size}.pth")

        plots_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/figures/combined/{data_version}/binnorm"
