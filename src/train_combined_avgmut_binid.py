...
def train_combined(data_version):
    ...
    print("Training on average mutation rate and bin ID")
    print(f"Version of the data: {data_version}")

    X_local_train, X_avg_mut_1mb_train, X_bin_id_1mb_train, y_train, X_local_val, X_avg_mut_1mb_val, X_bin_id_1mb_val, y_val, _, _, X_bin_id_1mb_test, _ = load_splits(data_version, requested_splits=["train", "val", "test"], bin_size="1mb", requested_features=["avg_mut", "bin_id"])
    ...
    train_dataset = TensorDataset(X_local_train, X_avg_mut_1mb_train, X_bin_id_1mb_train, y_train)
    val_dataset = TensorDataset(X_local_val, X_avg_mut_1mb_val, X_bin_id_1mb_val, y_val)
    ...
    model = ModularModel(use_avg_mut=True, use_bin_id_embed=True, num_bins=n_bins, embed_dim=64)

    print("Shape of local context feature:", X_local_train[:2].shape)
    print("Shape of bin ID feature:", X_bin_id_1mb_train[:2].shape)
    with torch.no_grad():
        model(local_context=X_local_train[:2], avg_mut=X_avg_mut_1mb_train[:2],  bin_id=X_bin_id_1mb_train[:2])
    ...
    for epoch in range(epochs):
        ...
        model.train()
        running_loss = 0.0
        for xb_local, xb_avg_mut, xb_bin_id, yb in train_loader:
            pred = model(local_context=xb_local, avg_mut=xb_avg_mut, bin_id=xb_bin_id)
        ...
        with torch.no_grad():
            for xb_local, xb_avg_mut, xb_bin_id, yb in val_loader:
                pred = model(local_context=xb_local, avg_mut=xb_avg_mut, bin_id=xb_bin_id)
                loss = loss_func(pred, yb)
