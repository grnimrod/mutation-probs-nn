...
def train_combined(data_version):
    ...
    print("Training on bin ID and normalized bin ID")
    print(f"Version of the data: {data_version}")

    X_local_train, X_bin_id_1mb_train, y_train, X_local_val, X_bin_id_1mb_val, y_val, _, X_bin_id_1mb_test, _ = load_splits(data_version, requested_splits=["train", "val", "test"], bin_size="1mb", requested_features=["bin_id"])
    ...
    train_dataset = TensorDataset(X_local_train, X_bin_id_1mb_train, X_bin_id_1mb_normalized_train, y_train)
    val_dataset = TensorDataset(X_local_val, X_bin_id_1mb_val, X_bin_id_1mb_normalized_val, y_val)
    ...
    model = ModularModel(use_bin_id_embed=True, use_bin_id_norm=True, num_bins=n_bins, embed_dim=64)

    with torch.no_grad():
        model(local_context=X_local_train[:2], bin_id=X_bin_id_1mb_train[:2], bin_id_norm=X_bin_id_1mb_normalized_train[:2])
    ...
    for epoch in range(epochs):
        ...
        model.train()
        running_loss = 0.0
        for xb_local, xb_bin_id, xb_bin_id_norm, yb in train_loader:
            pred = model(local_context=xb_local, bin_id=xb_bin_id, bin_id_norm=xb_bin_id_norm)
        ...
        with torch.no_grad():
            for xb_local, xb_bin_id, xb_bin_id_norm, yb in val_loader:
                pred = model(local_context=xb_local, bin_id=xb_bin_id, bin_id_norm=xb_bin_id_norm)
                loss = loss_func(pred, yb)
