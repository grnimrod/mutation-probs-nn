...
def train_combined(data_version):
    ...
    print("Training on average mutation rate and normalized bin ID")
    ...
    for bin_size in bin_sizes:
        ...
        train_dataset = TensorDataset(X_local_train, X_avg_mut_1mb_train, X_bin_id_1mb_normalized_train, y_train)
        val_dataset = TensorDataset(X_local_val, X_avg_mut_1mb_val, X_bin_id_1mb_normalized_val, y_val)
        ...
        model = ModularModel(use_avg_mut=True, use_bin_id_norm=True)

        with torch.no_grad():
            model(local_context=X_local_train[:2], avg_mut=X_avg_mut_1mb_train[:2],  bin_id_norm=X_bin_id_1mb_normalized_train[:2])
        ...
        for epoch in range(epochs):
            ...
            model.train()
            running_loss = 0.0
            for xb_local, xb_avg_mut, xb_bin_id_norm, yb in train_loader:
                pred = model(local_context=xb_local, avg_mut=xb_avg_mut, bin_id_norm=xb_bin_id_norm)
            ...
            with torch.no_grad():
                for xb_local, xb_avg_mut, xb_bin_id_norm, yb in val_loader:
                    pred = model(local_context=xb_local, avg_mut=xb_avg_mut, bin_id_norm=xb_bin_id_norm)
                    loss = loss_func(pred, yb)
        ...
        model_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/models/combined/{data_version}/avgmut_binnorm"
        os.makedirs(model_dir, exist_ok=True)
        torch.save(model.state_dict(), f"{model_dir}/model_{bin_size}.pth")

        plots_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/figures/combined/{data_version}/avgmut_binnorm"
