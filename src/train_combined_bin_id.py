...
def train_combined(data_version):
    ...
    for bin_size in bin_sizes:
        ...
        train_dataset = TensorDataset(X_local_train, X_bin_id_1mb_train, y_train)
        val_dataset = TensorDataset(X_local_val, X_bin_id_1mb_val, y_val)
        ...
        model = ModularModel(use_bin_id_embed=True, num_bins=n_bins, embed_dim=64)

        print("Shape of local context feature:", X_local_train[:2].shape)
        print("Shape of bin ID feature:", X_bin_id_1mb_train[:2].shape)
        with torch.no_grad():
            model(local_context=X_local_train[:2], bin_id=X_bin_id_1mb_train[:2])
        ...
        for epoch in range(epochs):
            ...
            for xb_local, xb_region, yb in train_loader:
                pred = model(local_context=xb_local, bin_id=xb_region)
        ...
        model_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/models/combined/{data_version}/binid"
        os.makedirs(model_dir, exist_ok=True)
        torch.save(model.state_dict(), f"{model_dir}/model_{bin_size}.pth")

        plots_dir = f"/faststorage/project/MutationAnalysis/Nimrod/results/figures/combined/{data_version}/binid"
